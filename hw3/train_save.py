...
def train(X, Y, valX, valY):
    ...
    adam = Adam(lr=0.003)
    filepath = "selfmodel2_{epoch:02d}-{val_acc:.3f}.hdf5"
    ...
    if valX is not None:
        model.fit_generator(datagen.flow(X, Y, batch_size=_batch_size), steps_per_epoch=(10*len(X)/_batch_size) , epochs=500, validation_data=(valX, valY), callbacks= [checkpoint])
    else:
