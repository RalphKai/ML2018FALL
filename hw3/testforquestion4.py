...
def train(X, Y, valX, valY):
    ...
    model.add(Convolution2D(6, (2, 2),padding= 'valid',strides=(3,3)))
    model.add(Activation('relu'))
    # model.add(Dropout(0.3))
    # model.add(MaxPooling2D((2, 2)))
    # 
    model.add(Convolution2D(4, (2, 2),padding= 'valid',strides=(2,2)))
    # model.add(Activation('relu'))
    # model.add(MaxPooling2D((2, 2)))
    # # # model.add(Dropout(0.3))
    ...
    if valX is not None:
        model.fit_generator(datagen.flow(X, Y, batch_size=_batch_size), steps_per_epoch=(1*len(X)/_batch_size) , epochs=1, 
